...
```
resume_builder/
├── frontend/                 ← React lives here (editor + preview UI)   [to be built]
│
├── backend/                  ← Python (FastAPI) lives here              ✅ BUILT
│   ├── app/                  ← layered code (see BACKEND.md)
│   │   ├── main.py           ← the waiter: wires routes + CORS
│   │   ├── core/             ← config (pdflatex path, timeout, limits)
│   │   ├── api/routes/       ← health (GET /) + compile (POST /compile)
│   │   ├── models/           ← request/response schemas
│   │   ├── services/         ← latex_service.py — runs pdflatex (the heart)
│   │   └── utils/            ← reusable file + error helpers
...
Then open **http://localhost:8000/docs** to try `/compile` in the browser (no frontend needed yet).

---

## Build order (recommended)

1. ✅ **Backend first** — Python + LaTeX making a PDF (DONE — FastAPI `/compile`, tested).
   (The first PDF popped out — the whole thing "clicked".)
